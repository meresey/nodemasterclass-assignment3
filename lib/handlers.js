...
handlers.menuall = ({ headers, method, queryStringObject }, callback) => {
  ...
  // call the menu API
  // handlers.menu({ queryStringObject, headers }, (statusCode, menuData) => {
  //   // check for errors
  //   if (statusCode !== 200) return callback(statusCode)

  //   // build menu data
  //   const tdata = Object.keys(menuData).map(key => {
  //    return ( `<tr>
  //       <td><img src="${key}.jpg" /></td>
  //       <td>${menuData[key].name}</td>
  //       <td>${menuData[key].description}</td>
  //       <td>${menuData[key].price.regular}</td>
  //       <td>${menuData[key].price.medium}</td>
  //       <td>${menuData[key].price.large}</td>
  //       <td>${menuData[key].price.mega}</td>
  //     </tr>`)
  //   })
  //   const table = `
  //   <div class="container">
  ...
  //   </div>
  //   `
  //   helpers.getTemplate('menu', table, (error, template) => {
  //     if (error) return callback(404, { 'Error': error }, 'html')
  //     // return page
  //     callback(200, template, 'html')
  //   })
